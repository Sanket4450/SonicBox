...
const addRemoveLibraryPlaylist = async (
  token: string,
  { playlistId, isAdded }: addRemoveLibraryPlaylist
): Promise<void> => {
  try {
    ...
    if (isAdded) {
      const { isPrivate } = await playlistService.checkPlaylistPrivate(
        playlistId
      )
...
const addRemoveLibraryArtist = async (
  token: string,
  { artistId, isAdded }: addRemoveLibraryArtist
): Promise<void> => {
  try {
    const { sub } = await tokenService.verifyToken(
      token,
...
const addRemoveLibraryAlbum = async (
  token: string,
  { albumId, isAdded }: addRemoveLibraryAlbum
): Promise<void> => {
  try {
    const { sub } = await tokenService.verifyToken(
      token,
...
const getUserLibraryPlaylists = async ({
  userId,
  page,
  limit,
...
const getUserLibraryArtists = async ({
  userId,
  page,
  limit,
}: userIdPageAndLimit): Promise<artist[]> => {
  try {
    page ||= 1
    limit ||= 10
...
const getUserLibraryAlbums = async ({
  userId,
  page,
  limit,
}: userIdPageAndLimit): Promise<album[]> => {
  try {
    page ||= 1
    limit ||= 10
