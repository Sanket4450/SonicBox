...
const getUserByUsername = async (
  username: string
): Promise<Pick<getUserData, '_id' | 'role' | 'password'>> => {
  const query = {
    username,
  }
...
const getFullUser = async (
  userQuery: Partial<idUsernameEmail>,
  userData: Partial<selectUserData>
): Promise<Partial<getUserData>> => {
  const query = {
    ...userQuery,
  }
...
const updateUserById = async (
  ...
  try {
    ...
    if (userData.username || userData.email) {
      if (
        await authService.checkUserAlreadyExist(
    ...
    userData.role =
      userData.role && userData.role === 'admin'
        ? authService.validateUserRole(
            roleType.ADMIN,
            userData.secret as string
          )
        : userData.role && userData.role === 'artist'
        ? authService.validateUserRole(
            roleType.ARTIST,
            userData.secret as string
          )
...
const validateSession = async ({
  ...
  try {
    ...
    if (
      !(await DbRepo.findOne(constants.COLLECTIONS.SESSION, { query, data }))
    ) {
...
const getUserAndFollower = async (
  followingId: string,
  followerId: string
): Promise<{ _id: 1 } | null> => {
  try {
...
const followUnfollowUser = async (
  token: string,
  { userId, isFollowed }: followUnfollowUser
): Promise<void> => {
  try {
    const { sub } = await tokenService.verifyToken(
      token,
...
const getUserFollowers = async ({
  ...
  page,
  limit,
}: userIdPageAndLimit): Promise<user[]> => {
  try {
    page ||= 1
    limit ||= 10
...
const verifyUser = async (
  token: string,
  { userId, isVerified }: verifyUserInput
): Promise<void> => {
  try {
    const { sub, role } = await tokenService.verifyToken(
      token,
